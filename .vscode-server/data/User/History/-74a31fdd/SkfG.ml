...
let lex (str : string) =
  ...
  let tokenize (str : string) : string list =
    let rec tokenize' char_lst state str str_lst =
      ...
      | h :: t, Q1 -> if (transfer state h) = Q1 then tokenize' t (transfer state h) (str^(String.make 1 h)) str_lst (*Q1에서 digit이 입력되는 경우*) else tokenize' t (transfer state h) (String.make 1 h) (str::str_lst) (*Q1에서 operator이 들어오는 경우 *)
